...
- Fresh retry сохраняет прежнюю единственную попытку только для явно отклонённого `send` или подтверждённой отмены.
- Зависший до разрешения `send` считается потенциально принятым сервером: retry блокируется, пока `cancel` не подтверждён.
