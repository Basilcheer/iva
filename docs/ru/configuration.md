...
То, что Iva говорит сама, живёт в `data/settings.json`, а не в `.env` - эти ключи пишет экран **🔔 Уведомления** в `/menu`, а каждое расписание читает их в момент запуска, поэтому тап срабатывает без перезапуска.
...
Алерты о проблемах (память не бэкапится, ночной проход не прошёл) и офферы обновлений приходят всегда: их нельзя выключить, но каждый говорит, что делать, и повторяется не чаще раза в неделю на проблему - [ADR-0007](../adr/0007-notices-are-opt-in.md).
