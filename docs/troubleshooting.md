...
`iva doctor` only reports a missing vault origin — the repo creation and push happen in the nightly Brain pass (`iva-memory-doctor.service`); the second command runs it immediately instead of waiting for 05:00.
