...
или требуемое действие: тревоги brain, предложение обновиться) не выключается — но взамен обязан
...
которые оставляет только свёртка: курсор сессии любого периода в `data/`, запись периода в
`data/rollup-status.json`, дневные сводки в `vault/summaries/daily/`. Первого мало (его сносит
собственная уборка зависшего хода), второго — тоже (файл появился недавно), поэтому берётся
дизъюнкция. Установка ≤0.3.9 без vault под рукой следов не оставит и Notice не получит:
отчёты просто замолчат, объяснение — в CHANGELOG. Цена ошибки в эту сторону — молчание вместо
объяснения; в обратную — нотация человеку, который отчётов не видел.
