...
| `iva doctor` | Checks Node ≥ 24, `.env` keys, build, units, both services, two systemd watchdog timers, five eve schedules, vault git origin — auto-repairs what's safe |
| `iva status` | Status of both services, two systemd watchdog timers and five in-process eve schedules |
