...
- Карточки памяти ведут честный архив: историю и журнал пишет только сам инструмент, поэтому модель не может выдумать прошлое, которого не было. Повторные и устаревшие правки отклоняются вместо стопки дублей, а ночной `brain` показывает карточки с незакрытым код-фенсом.
