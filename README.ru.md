...
<details>
<summary><b>v0.3.19 · 12.08.2026 - развернуть последние релизы</b></summary>
...
- Обновление больше не ставится поверх работающей версии. Новая собирается рядом, проверяется живым запуском до того, как что-то переключится, и включается мгновенно. Апдейт оборвался на полпути - рабочая установка цела, а `iva rollback` вернёт предыдущую версию.
- Данные (`data/`, `vault/`, `.env`) переехали из кодового дерева наружу, а ваши файлы из `data/custom` накладываются при сборке версии. Сломанный кастом больше не кладёт сервис: Ива стартует на стоковой сборке и сообщает об этом.
- Карточки памяти ведут честный архив: историю и журнал пишет только сам инструмент, поэтому модель не может выдумать прошлое, которого не было. Повторные и устаревшие правки отклоняются вместо стопки дублей, а ночной проход Brain показывает карточки с незакрытым код-фенсом.
