...
- 🔔 **Утренние отчёты памяти больше не приходят без спроса** - ночная свёртка пишет vault молча. Отчёт в Telegram включается в `/menu` → 🔔 Уведомления. Там же тумблер утреннего дайджеста. Тумблер применяется той же ночью, без перезапуска. Установка, которая отчёт получала, один раз услышит, где его вернуть. Свежая установка не услышит ничего.
- 🗣️ **Iva больше не говорит наполовину по-английски** - язык любого своего сообщения она берёт из одного места: настройка языка, потом `AGENT_LANGUAGE`. Это касается ночного отчёта, тревог brain и оффера обновления. Отчёт стал человеческой запиской на 3-5 строк. Внутренних слов вроде SUPERSEDE в нём больше нет.
- 📨 **Отчёт приходит одним сообщением, а не двумя** - ночная сборка памяти отдаёт отчёт финальным текстом хода, доставку делает код. Правило записано и в промпте сборки, и в системном блоке инструкций: rich message в этих ходах запрещён.
- 🚨 **Тревоги стали полезными** - каждая говорит, что сломалось, чем это грозит и что сделать. Одна и та же проблема не повторяется чаще раза в неделю. Изменилась проблема или вернулась после починки - тревога придёт сразу. Битое состояние дросселя не заглушает тревогу.
