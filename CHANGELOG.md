...
Fix: memory cards survive updates, recovery actually recovers, the rich-post skill ships in the repo — plus a batch of reliability and permissions hardening. Big thanks to the contributors behind #34–#43.
...
- 🔐 **Secrets and files are private now** — `.env` is written 0600, every iva unit runs with `UMask=0077`, and updates chmod `.env`/`data/` on old installs. The gws login flow reaps its detached process and the temp log with the OAuth URL.
