...
// Unreleased, docs/deploy.md, scripts/lib/schedule-migration.mjs): catches a future
...
// changes what `eve info` reports and what schedule-migration.mjs's catch-up covers).
...
test("deploy/ contains none of the 8 retired iva-memory-{daily,weekly,monthly,yearly} unit files", () => {
  ...
  const retired = files.filter((f) => /^iva-memory-(daily|weekly|monthly|yearly)\.(service|timer)$/.test(f));
  assert.deepEqual(retired, [], "these units moved to agent/schedules/*.ts — see scripts/lib/schedule-migration.mjs");
...
test("agent/schedules/ contains exactly the 5 expected eve schedules", () => {
  const files = readdirSync(join(ROOT, "agent/schedules")).filter((f) => f.endsWith(".ts") || f.endsWith(".md"));
  assert.deepEqual(
    [...files].sort(),
    ["digest.ts", "memory-daily.ts", "memory-monthly.ts", "memory-weekly.ts", "memory-yearly.ts"].sort(),
  );
});
