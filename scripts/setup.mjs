...
async function main() {
  ...
  if (provider === "ollama") {
    ...
    if (auth) {
      ...
      while (!auth) {
        try {
          ...
          console.log(
            `  ${C.r}${t("sign-in failed", "не удалось войти")}: ${e.message}${C.x}`,
          );
          if (
            !(await askYesNo(
    ...
    if (auth) {
      try {
        ...
        console.log(
          `  ${C.y}${t("couldn't fetch the model list", "не смог получить список моделей")}: ${e.message}${C.x}`,
        );
      }
    }
    if (models.length) {
    ...
    `  ${t("Paste the Bot token", "Вставьте Bot token")}`,
    {
      ...
      validate: async (token) => {
        try {
          ...
          return t(
            `Telegram rejected the token (${e.message}). Copy it again from @BotFather.`,
            `Telegram не принял токен (${e.message}). Скопируйте заново у @BotFather.`,
          );
        }
      },
    },
  ...
  if (me?.username)
    console.log(`  → ${t("bot", "бот")}: ${C.g}@${me.username}${C.x}`);
  ...
  while (ids.size === 0) {
    ...
    try {
      ...
      console.log(
        `${C.y}  ${t(`Couldn't fetch updates: ${e.message}`, `Не смог получить апдейты: ${e.message}`)}${C.x}`,
      );
    }
    if (ids.size === 0) {
