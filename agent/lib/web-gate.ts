...
// Отсюда surface: "web" у санитайзера. Оно значит три вещи разом. Первое: гейт
// отдаёт текст даже при блокировке — по умолчанию он обнуляет его на
// invisible-flood и wallet-drain, а для страницы это молчаливая потеря
// содержимого. Второе: дорогие письменности (тибетский, Йи, Брайль,
// математические глифы) не вырезаются — на странице это контент, а не атака;
// расход держит бюджет в 2000 таких символов, поэтому модель получает статью с
// предупреждением, а не строку пробелов. Третье:
// правила читают все три языка, на которых агент реально читает веб, и ещё
// NFKC-вид текста — раз глифы остаются, маскировку математической латиницей
// снимает он. На блокирующем Telegram-входе такой набор правил ел бы обычные
// русские просьбы владельца, поэтому там всё осталось как было.
//
